    ...
    @objc func search(_ keyword: String) {
        ...
        case .user:
            ...
            if let handler = OIMApi.queryFriendsWithCompletionHandler {
                ...
                APIService.shared.searchUser(keyword) { [weak self] json in
                    guard let `self` = self else { return }
                    let info = FullUserInfo()
                    info.showName = json["NickName"].stringValue
                    info.userID = json["Id"].intValue.description
                    info.faceURL = json["FaceURL"].stringValue
                    info.gender = json["Gender"].intValue == 1 ? .male : .female
                    
                    self.userInfo = info
                    let uid = self.userInfo?.userID
