...
class UserDetailTableViewController: UIViewController {
    ...
    private lazy var sendMessageBtn: UIButton = {
        ...
        v.backgroundColor = .systemRed//UIColor.c0089FF
...
//                }
//            })
            MessageManager.shared.getConversation(sessionType: .c2c, sourceId: self._viewModel.userId) { conversation in
                MainTabBarController.shared.jumpConversation(conversation)
            }
        }).disposed(by: _disposeBag)
        return v
    }()
