...
class ChatViewModel: BaseViewModel {
    ...
    func fetchMessages(completion: @escaping (Bool) -> Void) {
        ...
        OIMManager.manager.getAdvancedHistoryMessageList(param) { [weak self] info in
            ...
            self.messages = info.messageList.filter({ msg in
                msg.typingElem == nil
            })
            self.lastMinSeq = info.lastMinSeq
            self.listIsEnd = info.isEnd
            self.startClientMsgID = info.messageList.first?.clientMsgID ?? ""
            completion(true)
    ...
    func fetchMoreList(completion: @escaping (Bool) -> Void) {
        ...
        OIMManager.manager.getAdvancedHistoryMessageList(param) { [weak self] info in
            ...
            self.cellHeightCache.removeAll()
            self.messages.insert(contentsOf: info.messageList.filter({ msg in
                msg.typingElem == nil
            }), at: 0)
            self.lastMinSeq = info.lastMinSeq
            self.listIsEnd = info.isEnd
    ...
    func makeMessageReaded(by: [OIMReceiptInfo], completion: @escaping () -> Void) {
        var messageIds: [String] = []
        _ = by.compactMap { info in
            messageIds.append(contentsOf: info.msgIDList ?? [])
        }
        let ms = messages.filter { info in
            messageIds.contains(info.clientMsgID ?? "")
        }
