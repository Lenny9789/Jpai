...
class MessageConversationCell: TableViewBaseCell {
    ...
    func present(_ model: OIMConversationInfo) {
        ...
        avatar.setImage(
            ...
            placeholderImage: UIImage(.systemBlue, content: model.showName ?? "", width: 40)
        )
        dateLabel.text = tt_timeStampToCurrentTime(timeStamp: model.latestMsg?.sendTime ?? 0)
